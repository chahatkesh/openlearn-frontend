...
const ProgressDashboard = ({ user, refreshTrigger = 0 }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
          <Target className="mr-2 text-[#FFDE59]" size={20} />
          Learning Milestones
        </h3>
            ...
            {[25, 50, 75, 100].map((milestone) => (
              <div key={milestone} className="flex items-center">
                <div className={`w-4 h-4 rounded-full mr-3 ${
                  stats.overallProgress >= milestone 
                    ? 'bg-[#FFDE59]' 
                    : 'bg-gray-200'
                }`}>
                  {stats.overallProgress >= milestone && (
                ...
                <span className={`text-sm ${
                  ...
                  {stats.overallProgress >= milestone && (
                    <span className="text-[#FFDE59] ml-1">✓</span>
                  )}
                </span>
              </div>
            ))}
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {dashboardData.enrollments.length > 0 
                    ? new Date(dashboardData.enrollments[0].enrolledAt).toLocaleDateString()
                    : 'N/A'
                  }
                </span>
              </div>
              {stats.specializationsCompleted > 0 && (
                <div className="flex justify-between">
                  <span>Specializations:</span>
                  <span className="font-medium text-[#FFDE59]">{stats.specializationsCompleted}</span>
                </div>
              )}
            </div>
          </div>
